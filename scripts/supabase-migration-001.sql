...
-- =============================================================================
-- 1. firms  (no FK to users — users doesn't exist yet)
-- =============================================================================
...
-- =============================================================================
-- 2. users  (FK to firms — firms now exists)
-- =============================================================================
...
-- =============================================================================
-- 3. All other tables  (columns that reference users omit the FK for now)
-- =============================================================================

-- firm_config — updated_by FK added later via ALTER TABLE
CREATE TABLE IF NOT EXISTS public.firm_config (
  ...
  updated_at            timestamptz DEFAULT now(),
  updated_by            uuid        -- FK added below after users exists
);
...
-- fee_earner_overrides — updated_by FK added later via ALTER TABLE
...
-- audit_log — user_id FK added later via ALTER TABLE
CREATE TABLE IF NOT EXISTS public.audit_log (
  id          uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  firm_id     uuid        REFERENCES public.firms(id) ON DELETE CASCADE,
  user_id     uuid,       -- FK added below after users exists
...
-- =============================================================================
-- 4. ALTER TABLE — add FKs that reference users (now that users exists)
-- =============================================================================

ALTER TABLE public.firm_config
  ADD CONSTRAINT firm_config_updated_by_fkey
  FOREIGN KEY (updated_by) REFERENCES public.users(id);

ALTER TABLE public.fee_earner_overrides
  ADD CONSTRAINT fee_earner_overrides_updated_by_fkey
  FOREIGN KEY (updated_by) REFERENCES public.users(id);
...
-- =============================================================================
-- 6. Helper functions  (defined after all tables so public.users exists)
-- =============================================================================
...
CREATE POLICY "firms_select" ON public.firms
  FOR SELECT
  USING (id IN (SELECT firm_id FROM public.users WHERE id = auth.uid()));
